...
from train_segmentation import build_input_pipeline, build_model

def confusion_matrix(prediction, targets, mask):
...
def test(test_split_path, test_tar_path, dict_path, log_dir, batch_size, vocab_size, n_oov_buckets, print_interval):
    ...
    # input pipelines
    # test
    ids, seq_lens, targets = build_input_pipeline(test_split_path, test_tar_path, vocabulary, batch_size, shuffle=False, allow_smaller_final_batch=True, num_epochs=1)
    ...
    conf_matrix = confusion_matrix(prediction, targets, mask)
    ...
    with tf.Session(config=config) as sess:
        ...
        checkpoint_dir = os.path.join(log_dir, 'best_model')
        ...
        try:
            while not coord.should_stop():
                n, loss_val, confus_matrix = sess.run([n_samples, loss, conf_matrix])
