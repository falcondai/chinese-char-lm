...
def build_model(token_ids, embeddings, seq_lens, vocab_size, embed_dim, rnn_dim):
    # encoder
    bs = tf.size(seq_lens)
    ...
    # rnn
    forward_cell = tf.contrib.rnn.LSTMCell( num_units=rnn_dim, 
                                            use_peepholes=False, 
                                            cell_clip=None, 
    ...
    backward_cell = tf.contrib.rnn.LSTMCell(num_units=rnn_dim, 
                                            use_peepholes=False, 
                                            cell_clip=None, 
                                            initializer=tf.contrib.layers.xavier_initializer(dtype = tf.float32), 
                                            num_proj=None, 
    ...
    rnn_output = output_fw + output_bw
...
def train(train_split_path, train_tar_path, val_split_path, val_tar_path, embedding_path, dict_path, log_dir, batch_size, vocab_size, n_oov_buckets, initial_lr, lr_decay_steps, lr_decay_rate, lr_staircase, no_grad_clip, clip_norm, opt_method, momentum, val_interval):
    ...
    with tf.variable_scope('model'):
        seq_logits, final_state = build_model(ids, embeddings, seq_lens, vocab_size + n_oov_buckets, embed_dim, rnn_dim)

    # validation
    with tf.variable_scope('model', reuse=True):
        val_seq_logits, val_final_state = build_model(val_ids, embeddings, val_seq_lens, vocab_size + n_oov_buckets, embed_dim, rnn_dim)
