...
def compute_embeddings(checkpoint_dir, dict_path, vocab_size, n_oov_buckets, embed_dim):
    ...
    id_emb_accu = np.zeros((1,vocab_size+n_oov_buckets, embed_dim))
    glyph_emb_accu = np.zeros((1, vocab_size+n_oov_buckets, embed_dim))
    ...
    with tf.Session(config=config) as sess:
        ...
        with open(dict_path, 'r') as fhandle:
            for i, line in enumerate(fhandle):
                ...
                id_emb_accu += id_emb_val
                glyph_emb_accu += glyph_emb_val

        id_emb_ave_norm = id_emb_accu / 4001.0
        glyph_emb_ave_norm = glyph_emb_accu / 4001.0

        print "id embedding average norm: ", np.linalg.norm(id_emb_ave_norm)   
        print "glyph embedding average norm: ", np.linalg.norm(glyph_emb_ave_norm)
