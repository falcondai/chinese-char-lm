...
def test(test_split_path, dict_path, log_dir, batch_size, vocab_size, n_oov_buckets, print_interval):
    ...
    with tf.variable_scope('model'):
        seq_logits, final_state, id_emb, cnn_emb = build_model(ids[:, :-1], glyph_ph[:, :-1], seq_lens, vocab_size + n_oov_buckets, embed_dim, rnn_dim, n_cnn_layers, n_cnn_filters)
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            while not coord.should_stop():
                ...
                n, loss_val, id_emb, cnn_emb = sess.run([n_samples, loss, id_input, cnn_input], {
                    ids: ids_val,
