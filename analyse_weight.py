...
import train_id_cnn_segmentation
import train_id_cnn_lm
...
def test(task_name, test_split_path, test_tar_path, dict_path, log_dir, batch_size, vocab_size, n_oov_buckets, print_interval):
    ...
    if task_name == "segmentation":
        build_input_pipeline = train_id_cnn_segmentation.build_input_pipeline
        build_model = train_id_cnn_segmentation.build_model
        ...
        build_input_pipeline = train_id_cnn_lm.build_input_pipeline
        build_model = train_id_cnn_lm.build_model
    ...
    with tf.variable_scope('model'):
        seq_logits, final_state, id_input, cnn_input = build_model(ids, glyph_ph, seq_lens, vocab_size + n_oov_buckets, embed_dim, rnn_dim, n_cnn_layers, n_cnn_filters)
    ...
    id_accumulative_vec = np.zeros(embed_dim)
    cnn_accumulative_vec = np.zeros(embed_dim)
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            while not coord.should_stop():
                ...
                n, loss_val, confus_matrix, id_emb, cnn_emb= sess.run([n_samples, loss, confusion_mat, id_input, cnn_input], feed_dict=feed)
                total_loss += n * loss_val
                total_n += n
                conf_mat += confus_matrix

                id_accumulative_vec += np.sum(np.sum(id_emb, axis = 1), axis = 0)
                cnn_accumulative_vec += np.sum(np.sum(cnn_emb, axis = 1), axis = 0)

                # record id_emb and cnn_emb
...
if __name__ == '__main__':
    ...
    parser.add_argument('--task_name', type=str, default='LM', help='language modeling task, segmentation stask')

    args = parser.parse_args()

    test(args.task_name, args.test_corpus, args.test_target, args.dictionary, args.log_dir, args.batch_size, args.vocab_size, args.n_oov_buckets, args.print_interval)
